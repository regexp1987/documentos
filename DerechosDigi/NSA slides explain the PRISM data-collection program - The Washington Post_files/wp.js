...
(function (win, doc, wpAd, undefined) {
  ...
  wpAd.config.init = function () {
    ...
    if(((!wpAd.flags.is_homepage && win.estNowWithYear >= 201305010000 && /^201305/.test(win.estNowWithYear)) || /testads\=samsung/.test(location.search)) && $){
      $(function(){
        $.ajax({
          cache: true,
          dataType: 'script',
  ...
  if(!wpAd.flags.no_ads) {
    ...
    if($){
      ...
      // add wp+ pixels
      // had to remove "jobs" from dom ready queue, presumably because jquery on jobs was being
      // overwritten after this point, but before dom ready
