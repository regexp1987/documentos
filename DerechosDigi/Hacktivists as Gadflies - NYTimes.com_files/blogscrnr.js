NYTD.Blogs.CommentCounts = (function($) {
	...
	function showCounts() {
		...
		if(postUrls.length == 0) {
			if(console) { console.log( "no urls on the page with .commentCountLink class" ); }
			return;
		}
		...
		$.ajax({
			...
			if(!blogsCommunityResponse.userContentSummary) {
				if(console) { console.log( "No response" ); }
				return;
			}
			...
			if(!assetsInResponse) {
				if(console) { console.log( "No assetsInResponse" ); }
				return;
			}
			...
			if(console) { console.log( "Comment Count Request failed: " + requestStatus ); }
		});
	}
